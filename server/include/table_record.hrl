...
-define(update_role_sql, <<"insert into roles(role_id,role_name,account_id,create_time,login_time) values('~ts','~ts','~ts',~p,~p) on duplicate key update role_id=values(role_id),role_name=values(role_name),account_id=values(account_id),create_time=values(create_time),login_time=values(login_time)">>).
