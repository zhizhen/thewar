...
connect(PoolId, Hostname, Username, Password, Database) ->
    mysql:connect(mysql_dispatcher, PoolId, Hostname, 3306, Username, Password, Database,
          undefined, true).
...
connect(PoolId, Hostname, Port, Username, Password, Database,
     Reconnect) ->
    mysql:connect(mysql_dispatcher, PoolId, Hostname, Port, Username, Password, Database,
          Reconnect).
...
transaction(Fun, Options) ->
    mysql:transaction(mysql_dispatcher, get_pool_id(Options), Fun, get_timeout(Options)).
...
prepare(Name, Stmt) ->
    mysql:prepare(mysql_dispatcher, Name, Stmt).
