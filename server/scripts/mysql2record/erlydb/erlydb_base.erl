...
field_from_string(ErlyDbField, Str) ->
    ...
    datetime ->
        Str1 = re:replace(Str,"-","/",[{return,list},global]),
        [Year, Month, Day, Hour, Minute, Second] =
                    fread_vals("~d/~d/~d ~d:~d:~d", Str1),
        {datetime, {make_date(Year, Month, Day),
            make_time(Hour, Minute, Second)}};
    date ->
        Str1 = re:replace(Str,"-","/",[{return,list},global]),
        [Year, Month, Day] = fread_vals("~d/~d/~d", Str1),
...
find(Module, Where, Extras) ->
    do_find(Module, field_names_for_query(Module, false), Where, Extras).
