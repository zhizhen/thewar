...
cc.Class({
    ...
    properties: {
        ...
        vehicle_speed: 0,
        status : 0, //0(初始状态)静止，1行驶,2等待 //2（被手指）触碰，4行驶并触碰
    ...
    update: function (dt) {
        ...
        if (this.move_sequence.length == 0 && this.status != 1){
            this.setStatus(0);
            return;
        }
        ...
        if (this.status != 1){
            ...
            console.log("update,x:" + location.x + ",y:" + location.y);
            this.move(location);
        }
    },
    
    setPosSequence : function(posSequence){
        console.log("setPosSequence");
        this.move_sequence = posSequence;
    },
    
    pushLocation: function(location) {
        this.move_sequence.unshift(location);        
    },
    
    popLocation: function(){
       var location = this.move_sequence.pop();
       return location;
    },
    ...
    move: function (position) {
        ...
        var spawn = cc.spawn(cc.moveTo(0.1, cc.p(x, y)));
        var finished = cc.callFunc(function(target, status){
            this.status = status
        }, this, 2);
        var sequence = cc.sequence(spawn, finished);
