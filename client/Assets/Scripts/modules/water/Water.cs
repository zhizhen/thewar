...
public class Water : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        // NOTE: Added by MichaelL on Oct 17, 2010 -- this makes the water world-position independent (also this function was changed to LateUpdate)
        Transform trans = Camera.main != null ? Camera.main.transform : transform;
        offsetClamped.x += trans.position.x * waveScale4.x;
        offsetClamped.y += trans.position.z * waveScale4.y;
        offsetClamped.z += trans.position.x * waveScale4.z;
        offsetClamped.w += trans.position.z * waveScale4.w;
        ...
        // We need to convert world position coordinates into 0-1 range UVs.
        Vector3 offset = (trans.position - Terrain.activeTerrain.transform.position);
        Vector3 size = Terrain.activeTerrain.terrainData.size;
        Vector4 os = new Vector4(offset.x, offset.z, 1.0f / size.x, 1.0f / size.z);
        mat.SetVector("_OffsetScale", os);
    }
